...
class ArrayValueLastDynamicReturnTypeExtension implements \PHPStan\Type\DynamicFunctionReturnTypeExtension
{

	public function isFunctionSupported(FunctionReflection $functionReflection): bool
	{
		return $functionReflection->getName() === 'array_value_last';
	}
	...
	public function getTypeFromFunctionCall(FunctionReflection $functionReflection, FuncCall $functionCall, Scope $scope): Type
	{
		...
		if (count($constantArrays) > 0) {
			...
			foreach ($constantArrays as $constantArray) {
				...
				$valueTypes[] = $arrayValueTypes[count($arrayValueTypes) - 1];
			}
